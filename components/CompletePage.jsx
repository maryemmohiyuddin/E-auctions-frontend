...
const handleRedirect= ()=>{
  window.location.href = "/bids";  // Redirect to the success page
  ...
  return (
    ...
    <div id="payment-status w-full flex flex-col items-center justify-center">
      {status && (
        <>
        <div className='flex items-center justify-center'>
      <span id="status-icon " className=' p-3' style={{backgroundColor: STATUS_CONTENT_MAP[status].iconColor, borderRadius:"100%"}}>
        {STATUS_CONTENT_MAP[status].icon}
      </span>
      </div>
      <h2 id="status-text " className='my-4 font-semibold text-xl text-center'>{STATUS_CONTENT_MAP[status].text}</h2>
    
      <a id="retry-button"   onClick={()=>handleRedirect()}>
        <button className='py-2  bg-secondary rounded-lg text-white w-full'>
