...
export default function Dashboard() {
	...
	const handleSearch = () => {
		...
		if (hasWord.test(searchQuery.trim())) {
			const searchedProducts = list.filter((li: any) => li.product.name.toLowerCase().includes(searchQuery.toLowerCase()))
	...
	const filterProducts = (auctionList: any = list) => {
		console.log('SelectedCat', selectedCat)

		console.log('in filter products', auctionList, selectedCat)
		const categoryNumber = Number(selectedCat)
		const filteredProducts = auctionList.filter((li: any) => li.product.category === categoryNumber)
	...
	return (
		<div>
				...
				<>
					<div className="mb-5  justify-between items-center  px-8 py-4 bg-secondary rounded-xl">
						<div className="text-white">
							...
							<p>
								Welcome to the Bidding Page! Here, you can explore a wide range of products available for bidding. Place your bids on items you’re interested in, track the current status of ongoing auctions, and stay updated on any
								changes. With user-friendly features and real-time notifications, you can easily manage your bids and participate in auctions with confidence. Good luck, and may the best bid win!
							</p>
						</div>
          ...
          <Button onPress={onOpen} className='bg-white  text-secondary' startContent={<IoMdAdd />}>
            Add Product
					...
					<div className="flex py-4 w-full sm:w-auto sm:justify-between sm:items-end sm-py-0">
						<div>
        ...
        >
          ▶
						...
						<div ref={scrollContainerRef} className="gap-5 p-2 mb-20 flex w-full overflow-x-auto scrollbar-hide">
							{searchCategory.map((item: any) => (
								<Card shadow="sm" key={item.id} isPressable className="min-w-[200px]" onPress={() => handleProductClick(item)}>
									<CardBody className="overflow-visible p-0">
										<Image isZoomed shadow="sm" radius="lg" width="100%" alt={item.product.name} className="w-full object-cover h-[140px]" src={item.product.picture} />
									</CardBody>
			...
			<div className="mt-5">
				<div className="flex flex-col gap-10 sm:flex-row">
					...
					<div className="w-full">
						...
						<div className="gap-5 mb-20 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 w-full">
							{products.map((item) => {
								...
								return (
									<Card shadow="sm" key={item.id} isPressable onPress={() => handleProductClick(item)}>
										...
										<CardFooter className="flex gap-1 pt-5 flex-col items-start justify-start text-sm">
											...
											<p>
												<span className="font-semibold">Starting Value:</span> Rs.{Number(item.starting_value)}
											</p>
											<p className="text-left">
												{' '}
												<span className="font-semibold"> Ending Time: </span> {format(new Date(item.ending_time), 'PPpp')}
											</p>
						...
						<ModalBody>
							{!hasPlacedBid && (
								<>
									...
									<Input label="Bidding Value" value={biddingValue} onChange={(e) => setBiddingValue(e.target.value)} type="number" min={selectedProduct.starting_value + 1} />
								</>
							)}
									...
									bids.map((bid: any, index: number) => {
										...
										return (
											<p className="mt-3 text-sm   " key={bid.id}>
												{position}
												{positionSuffix} place{isUserBid ? '(You) ' : ''}: {bid.bidding_value}
											</p>
										)
									})
								)}
